...
$sql_display_all = "SELECT * FROM users WHERE access_level = 4 AND id_team = $id";
...
$query3 = "SELECT product_name, SUM(product_quantity) AS total_quantity FROM order_items WHERE id_team = $id GROUP BY product_name";
$quantity = mysqli_query($connection, $query3);

// Function to calculate total quantity with unique product names
function calculateTotalQuantity($connection, $id) {
    $totalQuantity = 0;

    $query = "SELECT SUM(product_quantity) AS total_quantity FROM order_items WHERE id_team = $id";
...
$totalOrderedQuantity = calculateTotalQuantity($connection, $id);
...
function calculateTotalClient($connection, $id) {
    $totalClients = 0;

    $query = "SELECT COUNT(DISTINCT address) AS total_clients FROM orders WHERE id_team = $id";
...
$totalClients = calculateTotalClient($connection, $id);


// SQL query to calculate total amounts
$sql = "SELECT 
            (SELECT SUM(order_cost) FROM orders WHERE id_team = $id) AS total_price_sold,
            (SELECT SUM(order_cost) FROM orders WHERE order_status = 'Paid' AND id_team = $id) AS total_amount_collected,
            (SELECT SUM(order_cost) FROM orders WHERE order_status != 'Paid' AND id_team = $id) AS total_for_collection";
                 ...
                 <table id="example" class="table table-striped" style="width:100%">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th><center>Access Level</center></th>
                    ...
                    <tbody>
                         <?php while($rows = mysqli_fetch_array($query)){ ?>
                        <tr>
                            <th><center><?php echo $rows['id']; ?></center></th>
                            ...
                            <td><center><?php echo $rows['access_level']; ?></center></td>
                            <td><center><?php echo $rows['position']; ?></center></td>
                            <td><center>
                            <a class="btn btn-outline-primary" href="reports_view.php?id=<?php echo $rows['id']; ?>" role="button"><span class="material-symbols-outlined">view_timeline</span>View</a>
                            </center></td>
                        </tr>
                        <?php } ?>
                    </tbody>
